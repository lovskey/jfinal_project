...
function query() {
	...
	$('#datagrid').datagrid({
		url:$.navinfo.contextPath + "/TastyAction.do?method=getTastyList",
		width: 700,
		height: 400,
		striped: true,
		collapsible: true,
...
function modifyOperate() {
	...
	if (row) {
		var d = $.navinfo.dialog("修改会员信息", $.navinfo.contextPath + "/MemberAction.do?method=edit&KEYID="+row.KEYID, 700, 450).dialog('open'); 
		d.operate = "";
		d.dialog({
		    onClose:function(){
